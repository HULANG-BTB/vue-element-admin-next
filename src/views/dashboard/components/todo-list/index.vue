...
const filters: Record<string, Function> = {
  ...
  active: (todos: ITodo[]) => todos.filter((todo: ITodo) => !todo.done),
  completed: (todos: ITodo[]) => todos.filter((todo: ITodo) => todo.done)
}
...
export default defineComponent({
  ...
  setup() {
    ...
    const todos = ref<ITodo[]>(defalutList)

    const allChecked = computed(() => {
      return todos.value.every((todo: ITodo) => todo.done)
    })
    ...
    const remaining = computed(() => {
      return todos.value.filter((todo: ITodo) => !todo.done).length
    })

    const toggleAll = ({ done }: any) => {
      todos.value.forEach((todo: ITodo) => {
        todo.done = done
        setLocalStorage()
      })
    }
